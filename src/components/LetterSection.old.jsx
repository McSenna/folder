...
const LETTER_CONTENT = [
  'Happy Girlfriend\'s Day, my dearest lalab. ❤️🌹 Thank you for choosing to love me every single day, for believing in me even when I struggle to believe in myself, and for always being my greatest supporter. Thank you for caring for me with a love so genuine and selfless that it makes me feel like the luckiest man in the world. You are my safe place, my comfort, and one of the greatest blessings in my life.',
  '',
  'I know I may not be able to give you everything you deserve right now, but I promise that this is not where our story ends. One day, I\'ll make it all up to you. I\'ll repay every sacrifice, every understanding, every tear, and every moment you\'ve stayed by my side with a love that never stops choosing you. Until that day comes, please hold my hand a little longer, because every step I take is for us and for the beautiful future we\'ll build together.',
...
function LetterCard({ onClose, closeButtonRef }) {
  return (
    <div
      className="relative mx-auto max-w-2xl w-full max-h-[70vh] sm:max-h-[75vh] rounded-[20px] overflow-hidden"
      style={{
    ...
    >
      ...
      <div className="relative h-full overflow-hidden">
        <div className="sticky top-4 z-20 flex justify-end px-6 pt-6 sm:px-10">
          <button
            type="button"
            ref={closeButtonRef}
            onClick={onClose}
            className="inline-flex h-11 w-11 items-center justify-center rounded-full border border-white/70 bg-white/60 text-text-primary shadow-[0_16px_36px_rgba(120,75,75,0.16)] backdrop-blur-sm transition-transform duration-300 hover:-translate-y-0.5 hover:scale-[1.02] focus:outline-none focus-visible:ring-2 focus-visible:ring-rose-gold/60"
        ...
        <div
          className="relative px-6 pb-10 pt-4 sm:px-10 sm:pb-12 overflow-y-auto scroll-smooth scrollbar-hidden touch-pan-y"
          style={{
...
export default function LetterSection({ visible }) {
  ...
  useEffect(() => {
    ...
    if (!isModalOpen) {
      if (bodyStyleRef.current && typeof window !== 'undefined') {
    ...
    const trapFocus = (event) => {
      ...
      const focusableElements = Array.from(
        ...
        (element) => !element.hasAttribute('disabled') && element.getAttribute('aria-hidden') !== 'true' && (element.tabIndex ?? 0) >= 0,
      )
    ...
    >
      ...
      <div className="relative mx-auto max-w-3xl">
        ...
        {showLetter && (
          ...
          >
            <div
              className="w-full max-w-[680px] max-h-[70vh] sm:max-h-[75vh] overflow-hidden rounded-[24px]"
              style={{
                animation: isClosing
            ...
            >
              <LetterCard onClose={handleClose} closeButtonRef={closeButtonRef} />
            </div>
          </div>
        )}
      </div>
